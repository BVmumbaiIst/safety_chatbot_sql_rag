        ...
        if not user_question or len(user_question.strip()) == 0:
            ...
            try:
                ...
                if st.session_state.get("filtered_sql"):
                    ...
                    summary = None
                    try:
                        # generate a concise summary string for the LLM
                        numeric_summary = df_for_analysis.describe(include=[np.number]).transpose().round(2)
                        categorical_summary = {col: df_for_analysis[col].value_counts().head(5).to_dict() for col in df_for_analysis.select_dtypes(include='object').columns}
                        summary = f"Numerical Summary:\n{numeric_summary.to_string()}\n\nTop categories:\n{json.dumps(categorical_summary, indent=2)}"
                    except Exception:
                        summary = f"Rows: {len(df_for_analysis)}"

                    # detect relevance (simple heuristic if LLM not available)
                    ...
                    relevance = False
                    if llm:
                        # call LLM to check relevance (avoid large prompts; use small preview)
                        try:
                    ...
                    # Build analytical prompt for detailed answer
                    if llm:
                        # Compose task prompt
                        kpi_text = ""
                        try:
                            total_rows = len(df_for_analysis)
                            top_template = df_for_analysis["TemplateNames"].value_counts().idxmax() if "TemplateNames" in df_for_analysis.columns and not df_for_analysis["TemplateNames"].isna().all() else "N/A"
                            top_employee = df_for_analysis["owner name"].value_counts().idxmax() if "owner name" in df_for_analysis.columns and not df_for_analysis["owner name"].isna().all() else "N/A"
                            kpi_text = f"Total Records: {total_rows}\nTop Template: {top_template}\nTop Employee: {top_employee}"
                        except Exception:
                            pass
...
"""
                        try:
                            ...
                            if hasattr(llm_out, "content"):
                                answer_text = llm_out.content
                            elif isinstance(llm_out, dict) and "content" in llm_out:
                                answer_text = llm_out["content"]
                            else:
                    ...
                    # show sample table (won't keep large df in session)
                    st.markdown("### 🔎 Sample Data (used for analysis)")
                    st.dataframe(df_for_analysis.head(200))

                    # auto-generate visuals for the limited df_for_analysis
                    try:
                        auto_df = df_for_analysis
                        # simple visuals
                        if "region" in auto_df.columns and "TemplateNames" in auto_df.columns:
                            region_count = auto_df.groupby("region")["TemplateNames"].count().reset_index(name="count")
                            fig = px.bar(region_count, x="region", y="count", text="count", title="Inspections by Region")
...
with col_right:
    ...
    # show small preview if present
    preview = st.session_state.get("filtered_preview")
    if preview is not None and not preview.empty:
        st.markdown("### 🔍 Preview (small sample from Run Query)")
        st.dataframe(preview)

        # completion by month chart - limited computation
        try:
            if "date completed" in preview.columns:
                preview2 = preview.assign(
                    completion_month=pd.to_datetime(preview["date completed"], errors="coerce").dt.to_period("M").astype(str)
                )
                chart_df = preview2.groupby("completion_month")["TemplateNames"].count().reset_index(name="template_count").sort_values("completion_month")
                st.markdown("### 📅 Inspections by Completion Month (preview)")
                st.bar_chart(chart_df.set_index("completion_month")["template_count"])
        except Exception:
            pass

        # more visuals (use preview)
        try:
            if "TemplateNames" in preview.columns:
                template_count = preview["TemplateNames"].value_counts().head(10).reset_index()
                template_count.columns = ["TemplateNames", "count"]
                fig = px.bar(template_count, x="TemplateNames", y="count", text="count", title="Top Templates (preview)")
                st.plotly_chart(fig, use_container_width=True)
        except Exception:
            pass

    else:
        st.info("ℹ️ No preview available. Use the sidebar filters and click 'Run Query' to load a preview.")

# ============================================================
# Small utility: force GC & show memory usage (debug only)
# ============================================================
